...
def make_model_name(timestamp: datetime,
    ...
    Format:
    ts_label{params}_sigma_layers-dropouts_activation_loss_acc_optimizer
    """
    ...
    # jeżeli label to partial – pobierz jego parametry
    if hasattr(label, "func") and hasattr(label, "keywords"):
        base_name = label.func.__name__
        if label.keywords:
            params_str = ";".join(
                f"{k}={v}" for k, v in label.keywords.items()
            )
        else:
            params_str = ""
    ...
    # dropouts (jeśli podane)
    dropouts_str = "-".join(f"{d:.2f}" for d in dropouts) if dropouts else ""

    return f"{ts_str}_{label_str}_{sigma_str}_{layers_str}_{dropouts_str}_{activation}_{loss}_{acc_str}_{optimizer}"
...
def prepare_all_data(data_path, sigma, filters,
    ...
    for ticker_file in path.iterdir():
        if ticker_file.is_dir():
            ...
            # --- oblicz VWAP i ekstremalne punkty ---
            ...
            if recalc_features:
                ...
                feature_cols = [f"feature_{c}" for c in feature_cols]
                ...
                if features_path.exists():
                    ...
                    raise ValueError(f"Ścieżka {features_path} nie istnieje!")

            if label_func is not None:
                print(f"Obliczam label!")
                start_time_label = time()
                y_series = label_func(df).dropna()
                print("Label obliczony!")
                end_time_label = time()
                print(f"Policzenie labela zajęło {end_time_label - start_time_label}")
            else:
                print(f"Nie podano funkcji obliczającej label - próbuję pobrać dane z pliku parquet!")
                labels_path = ticker_file / "labels" / f"{labels_filename}.parquet"
                if labels_path.exists():
                    y_series = pd.read_parquet(labels_path).squeeze("columns")
                else:
                    raise FileNotFoundError(f"Brak pliku {labels_path}")

            # przycięcie df do indeksu etykiet
            df = df.loc[y_series.index].copy()

            print("Przechodzę do stosowania filtrów!")
            for f in filters:
                before = len(df)
                df = f(df)
                after = len(df)
                print(f"Filtr {f.__name__} → {before} → {after} rekordów "
                      f"({(before - after) / before:.2%} usunięto)")

            # --- ekstremalne punkty ---
            df = df[df["is_extreme"]].copy()

            if df.empty:
                raise ValueError("Ramka danych po filtrach i przycięciu ekstremalnych pusta!")
...
def build_model_random(input_dim):
    """
    Tworzy losowy model MLP do klasyfikacji binarnej (bardziej kontrolowany).
    Zasady:
      - 3–5 warstw ukrytych
    ...
    # układ warstw: losuj wariant
    if n_layers == 3:
        # klasyczny układ duża-duża-mała
        layer_sizes = [random.choice(big), random.choice(big), random.choice(small)]
    else:
        # układ mała-duża-duża-mała (dla 4–5 warstw)
    ...
    # dropouty – malejące, min 0.05, max 0.30
    start_dropout = round(random.uniform(0.2, 0.3), 2)
    step = round(random.uniform(0.03, 0.07), 2)
    dropouts = []
    for i in range(n_layers):
        d = max(0.05, round(start_dropout - i * step, 2))
        dropouts.append(d)

    # aktywacja
    activation = random.choice(['relu', 'tanh'])

    # optimizer – tylko Adam i RMSprop
    ...
    # loss – zawsze binary_crossentropy
...
def train(data_path, label, features_filename, batch_size):
    ...
    if max_class > expected * (1 + tolerance_error) or max_class < expected * (1 - tolerance_error):
        print(f"Wykryto nierówności w klasach!")
        print(f"Liczba klas: {n_unique}")
        print(f"Największy przydział klasy: {max_class}")
        print(f"Natomiast rozkład oczekiwaany to {expected}")
    ...
    # --- Budowa modelu ---
    model, meta = build_model_random(X_train.shape[1])
    # model.compile(
    #     optimizer=optimizer,
    #     loss='binary_crossentropy',
    #     metrics=['accuracy']
    ...
    model_name = make_model_name(datetime.now(), label, sigma_val, meta["layer_sizes"], acc, meta["optimizer"], dropouts=meta["dropouts"], activation=meta["activation"], loss=meta["loss"] )

    final_path = models_path + model_name + ".keras"
    ...
    print(classification_report(y_test, y_pred_classes, target_names=["0", "1"]))
...
label_1  = partial(calc_label9, T=35, alpha=0.72,  use_atr=False)
label_2  = partial(calc_label9, T=35, alpha=0.725, use_atr=False)
label_3  = partial(calc_label9, T=35, alpha=0.75,  use_atr=False)
label_4  = partial(calc_label9, T=35, alpha=0.775, use_atr=False)
label_5  = partial(calc_label9, T=35, alpha=0.8,   use_atr=False)

label_6  = partial(calc_label9, T=40, alpha=0.72,  use_atr=False)
label_7  = partial(calc_label9, T=40, alpha=0.725, use_atr=False)
label_8  = partial(calc_label9, T=40, alpha=0.75,  use_atr=False)
label_9  = partial(calc_label9, T=40, alpha=0.775, use_atr=False)
label_10 = partial(calc_label9, T=40, alpha=0.8,   use_atr=False)

label_11 = partial(calc_label9, T=45, alpha=0.72,  use_atr=False)
label_12 = partial(calc_label9, T=45, alpha=0.725, use_atr=False)
label_13 = partial(calc_label9, T=45, alpha=0.75,  use_atr=False)
label_14 = partial(calc_label9, T=45, alpha=0.775, use_atr=False)
label_15 = partial(calc_label9, T=45, alpha=0.8,   use_atr=False)


labels1 = [label_1, label_4, label_5]
labels2 = [label_6, label_9, label_10]
labels3 = [label_12, label_14, label_15]


data_path = "data/training_data"
features_filename = "features0"
