...
def backtest(
    ...
    round_trip_fee_frac = 2.0 * fee_rate * leverage  # frakcja od notional (wejście+wyjście)
    ...
    while i < n:
        ...
        # --- P&L w USD na stałym notional 1000 ---
        ...
        trades.append({
            ...
            "equity_after": float(equity_pnl)       # skumulowany P&L USD
        })
...
if __name__ == "__main__":
    ...
    y_true = df["label"].astype(int).values  # zakładam, że masz kolumnę "label"
    ...
    # --- Ewaluacja ---
    print("\n=== 📈 Accuracy vs Threshold (2-class softmax) ===")
    df_eval = evaluate_thresholds_softmax(y_true, probs)
    print(df_eval.to_string(index=False))

    # --- Dodatkowo: maksymalne accuracy i próg ---
