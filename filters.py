...
def filter_no_zero_inf_nan():
    """
    Usuwa wszystkie wiersze, w których którakolwiek kolumna ma:
    - 0.0
    - NaN
    - +inf / -inf

    Filtr 'drugiego rzędu' – stosować po obliczeniu lub wczytaniu featurów.
    """
    def _filter(df: pd.DataFrame) -> pd.DataFrame:
        df = df.copy()
        before = len(df)
        ...
        # maska na wiersze, gdzie wszystkie wartości != 0 i != NaN
        numeric_cols = df.select_dtypes(include=[np.number]).columns
        mask_valid = (df[numeric_cols] != 0).all(axis=1) & df[numeric_cols].notna().all(axis=1)

        df = df[mask_valid]
        after = len(df)
        print(f"filter_no_zero_inf_nan → {before:,} → {after:,} (usunięto {before - after:,})")
        return df

    _filter.__name__ = "filter_no_zero_inf_nan"
    return _filter

filters = [filter_hours(0, 8), filter_around_fomc()] #filter_remove_long_series(40, sigma_val), filter_clean()]